...
async def _property(conn, property_code: str):
    row = await conn.fetchrow(
        'SELECT id,timezone,currency FROM properties WHERE code=$1', property_code
    )
    if not row:
...
def _rows_to_schedule(rows) -> list[ScheduleSegment]:
    return [
        ScheduleSegment(room_id=row["roomId"], start=row["startDate"], end=row["endDate"])
        for row in rows
    ]
    ...
    if status == "CHECKED_IN":
        ...
        for night in _nights(reservation["checkIn"], history_end):
            if _night_room(current, night) != _night_room(proposed, night):
                raise HTTPException(
                    status_code=409,
                    detail={"code": "PAST_ROOM_HISTORY_IMMUTABLE", "date": night.isoformat()},
                )
