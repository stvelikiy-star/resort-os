...
def _require_webhook_secret(value: str | None) -> None:
    if not TELEGRAM_SALES_WEBHOOK_SECRET:
        raise HTTPException(status_code=status.HTTP_503_SERVICE_UNAVAILABLE, detail="Telegram Sales webhook is not configured")
...
def _content_type(message: dict[str, Any]) -> str:
    if message.get("text") is not None: return "TEXT"
    if message.get("voice") is not None: return "VOICE"
    if message.get("audio") is not None: return "AUDIO"
    if message.get("photo") is not None: return "PHOTO"
    if message.get("video") is not None: return "VIDEO"
    if message.get("video_note") is not None: return "VIDEO_NOTE"
    if message.get("document") is not None: return "DOCUMENT"
    if message.get("contact") is not None: return "CONTACT"
    if message.get("location") is not None: return "LOCATION"
    if message.get("sticker") is not None: return "STICKER"
    return "OTHER"
...
async def send_telegram_text(chat_id: str, text: str) -> dict[str, Any]:
    """Call Bot API sendMessage and return provider evidence.

    SENT = Telegram returned success and a Message.
    FAILED = Telegram returned a definite rejection.
    UNKNOWN = transport/timeout failure; automatic retry is unsafe because
    Bot API sendMessage has no client idempotency key.
    """
    if not TELEGRAM_SALES_BOT_TOKEN:
        return {"state": "FAILED", "description": "Telegram Sales outbound is not configured", "provider_status_code": None, "provider_payload": None, "message_id": None, "sent_at": None}

    url = f"{TELEGRAM_BOT_API_BASE_URL}/bot{TELEGRAM_SALES_BOT_TOKEN}/sendMessage"
    try:
        async with httpx.AsyncClient(timeout=TELEGRAM_PROVIDER_TIMEOUT_SECONDS) as client:
            response = await client.post(url, json={"chat_id": chat_id, "text": text})
    except httpx.RequestError as exc:
        return {"state": "UNKNOWN", "description": f"Telegram request transport error: {exc.__class__.__name__}", "provider_status_code": None, "provider_payload": None, "message_id": None, "sent_at": None}
    ...
    if response.is_success and data.get("ok") is True and isinstance(message_id, int):
        ...
        sent_at = datetime.fromtimestamp(unix_date, tz=timezone.utc) if isinstance(unix_date, int) and unix_date >= 0 else datetime.now(timezone.utc)
        return {"state": "SENT", "description": None, "provider_status_code": response.status_code, "provider_payload": data, "message_id": message_id, "sent_at": sent_at}

    description = data.get("description")
    if not isinstance(description, str):
        description = f"Telegram rejected sendMessage with HTTP {response.status_code}"
    return {"state": "FAILED", "description": description[:1000], "provider_status_code": response.status_code, "provider_payload": data, "message_id": None, "sent_at": None}
...
async def telegram_sales_webhook(
    ...
    if sender.get("is_bot") is True:
        return {"accepted": True, "ignored": True, "reason": "bot_sender", "update_id": update_id}
    ...
    sent_at = datetime.fromtimestamp(unix_date, tz=timezone.utc) if isinstance(unix_date, int) and unix_date >= 0 else None
    ...
    payload = NormalizedChannelMessage(
        idempotency_key=f"telegram:update:{update_id}", channel_code=TELEGRAM_SALES_CHANNEL_CODE,
        channel_kind="TELEGRAM", channel_display_name=TELEGRAM_SALES_DISPLAY_NAME,
        external_account_id=TELEGRAM_SALES_ACCOUNT_ID, external_conversation_id=chat_id,
        external_contact_id=sender_id, contact_name=_contact_name(sender), contact_phone=phone_number,
        contact_username=str(sender["username"])[:180] if sender.get("username") is not None else None,
        direction="INBOUND", external_message_id=f"{chat_id}:{message_id}", sender_type="GUEST",
        sender_external_id=sender_id, text=text, content_type=_content_type(message),
        delivery_status="RECEIVED", sent_at=sent_at, raw_payload=update,
    )
    result = await ingest_normalized_channel_message(payload, request, {"actor_type": "SERVICE", "actor_id": "telegram-sales-webhook"})
    return {"accepted": True, "ignored": False, "update_id": update_id, **result}
