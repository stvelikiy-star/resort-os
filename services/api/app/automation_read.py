...
async def hotel_facts(request: Request, _service: dict[str, Any] = Depends(service_access)):
    ...
    return {
        ...
        "sales_handoff": {
            "automation_goal": "HOT_QUALIFIED_LEAD",
            "manager_handles_prepayment": True,
            "automation_collects_prepayment": False,
            "automation_decides_prepayment_amount": False,
            "automation_decides_payment_method": False,
        },
        "reservation_rules": {
            ...
            "prepayment_amount_rule": "Manager decides and collects prepayment manually. Automation must not infer or negotiate the amount. If required_prepayment_kgs exists in Core, treat it only as a manager-entered/internal fact.",
        },
        ...
        "truth_rules": [
            ...
            "Automation qualifies the guest and hands the request to a manager; it does not collect prepayment.",
            "Do not claim payment is received without a manager-confirmed RECEIVED payment fact.",
            "Do not claim a room is booked without a Reservation fact.",
            "Do not infer prepayment amount, terms or payment method.",
...
async def reservation_request_status(
    ...
    return {
        "request": {
            ...
            "required_prepayment_truth": "Internal manager-entered fact only when present; automation must not infer or negotiate it.",
            "is_reservation": False,
        },
        "payments": {"received_kgs": payment["received_kgs"], "pending_kgs": payment["pending_kgs"]},
        "reservation": reservation,
        "truth": "The request itself is not a reservation. Automation hands qualified requests to a manager; use the reservation object, when present, for booking truth.",
    }
...
async def reservation_status(
    ...
    return {
        ...
        "truth": "Stored internal Resort Core reservation/payment facts only; payment collection itself is manager-owned in V1.",
    }
