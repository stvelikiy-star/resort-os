  ...
  return <>
    ...
    <main className="landing-v3" id="top">
      <section className="v3-hero" aria-labelledby="hero-title">
        <div className="v3-hero-media" aria-hidden="true"><video autoPlay muted loop playsInline preload="metadata" poster="/media/three-crowns/hero-resort.webp"><source src="/media/three-crowns/hero-video.mp4" type="video/mp4" /></video></div>
        <div className="v3-hero-shade" aria-hidden="true" />
        <div className="wrap v3-hero-content"><p className="eyebrow light">Три Короны · Resort & SPA · Чолпон-Ата</p><h1 id="hero-title">Иссык-Куль.<br />Ваш отдых начинается здесь.</h1><p className="v3-hero-copy">Курорт у самой воды: собственный пляж, 150-метровый пирс, SPA, открытый бассейн и 12 категорий размещения. Выберите даты — дальше мы поможем собрать отдых под вас.</p><div className="v3-hero-actions"><a className="button button-accent" href="#booking">Проверить свободные номера</a><Link className="button button-quiet" href="/rooms">Смотреть номерной фонд</Link></div><div className="v3-hero-meta" aria-label="Ключевые факты"><span><b>84</b> номера</span><span><b>12</b> категорий</span><span><b>150 м</b> пирс</span><span><b>15×8 м</b> бассейн</span></div></div>
        <a className="v3-scroll-cue" href="#booking" aria-label="Перейти к бронированию"><span>Начать</span><i>↓</i></a>
      </section>

      <section className="v3-booking" aria-labelledby="booking-experience-title"><div className="wrap v3-booking-heading"><div><p className="eyebrow">Бронирование без лишних шагов</p><h2 className="display-title" id="booking-experience-title">Сначала даты.<br />Потом — лучший вариант.</h2></div><div className="v3-booking-intro-copy"><p>Укажите даты и состав гостей. Система покажет свободные категории и стоимость за весь период. Если понадобится помощь — менеджер подключится на любом этапе.</p><div className="v3-booking-trust"><span>Актуальное наличие</span><span>Стоимость за период</span><span>Помощь менеджера</span></div></div></div><div className="wrap v3-booking-stage"><div className="v3-booking-main"><BookingWidget /></div><aside className="v3-booking-help" aria-label="Помощь с бронированием"><div><p className="eyebrow light">Нужна помощь?</p><h3>Подберём номер вместе</h3><p>Расскажите, кто едет, на сколько дней и какой отдых вы хотите. Менеджер поможет сравнить категории и дополнительные услуги.</p></div><div className="v3-help-actions"><a href="tel:+996558085002">Позвонить · +996 558 08 50 02</a><a href="https://wa.me/996558085008" target="_blank" rel="noreferrer">Написать в WhatsApp ↗</a></div><div className="v3-booking-rule"><strong>Важно</strong><p>Номер автоматически не блокируется после заявки. Подтверждённая бронь оформляется менеджером после согласования условий и предоплаты.</p></div></aside></div></section>

      <section className="v3-advantages" aria-labelledby="advantages-title"><div className="wrap v3-section-head"><div><p className="eyebrow">Почему Три Короны</p><h2 className="display-title" id="advantages-title">От номера<br />до воды — один маршрут</h2></div><p>Преимущества курорта не спрятаны в длинном списке. Они становятся последовательностью дня: проснуться, выйти на территорию, дойти до воды, отдохнуть у бассейна и закончить вечер в SPA.</p></div><div className="v3-advantage-marquee" aria-label="Преимущества курорта"><div className="v3-advantage-track">{loopingAdvantages.map((item, index) => <article className="v3-advantage-card" key={`${item.index}-${index}`} aria-hidden={index >= advantages.length}><div className="v3-advantage-image"><Image src={item.image} alt="" fill sizes="380px" /></div><div className="v3-advantage-body"><span>{item.index}</span><h3>{item.title}</h3><p>{item.text}</p></div></article>)}</div></div></section>

      <section className="v3-rooms" id="rooms" aria-labelledby="rooms-title"><div className="wrap v3-section-head"><div><p className="eyebrow">Номерной фонд</p><h2 className="display-title" id="rooms-title">Все 12 категорий.<br />Выбирайте спокойно.</h2></div><div><p>Каждая карточка ведёт на отдельную страницу категории: описание, вместимость, площадь, сезонные цены и форма проверки дат. Пока используем общий кадр номерного фонда — индивидуальные фотографии добавим после вашей загрузки.</p><Link className="text-link" href="/rooms">Открыть полный каталог →</Link></div></div><div className="wrap v3-room-grid">{roomCategories.map((room) => <Link className="v3-room-card" href={`/rooms/${room.slug}`} key={room.slug}><div className="v3-room-card-photo"><Image src="/media/three-crowns/room-double.webp" alt={room.name} fill sizes="(max-width: 760px) 88vw, (max-width: 1100px) 44vw, 31vw" /></div><div className="v3-room-card-body"><div className="v3-room-card-top"><span>{room.index}</span><span>{room.capacity} · {room.area}</span></div><h3>{room.name}</h3><p>{room.summary}</p><div className="v3-room-card-price"><small>Высокий сезон</small><strong>{formatKgs(room.rates.peak)} сом / сутки</strong></div><b>Смотреть категорию →</b></div></Link>)}</div></section>

      <section className="v3-territory" id="resort" aria-labelledby="territory-title"><div className="wrap v3-section-head light-head"><div><p className="eyebrow light">Территория курорта</p><h2 className="display-title light" id="territory-title">От первого шага<br />до конца пирса</h2></div><p>Раздел открывается большим видео о пансионате и показывает курорт как единый маршрут — от размещения до берега Иссык-Куля.</p></div><div className="wrap v3-territory-film"><video controls playsInline preload="metadata" poster="/media/three-crowns/hero-resort.webp"><source src="/media/three-crowns/resort-drone.mp4" type="video/mp4" /></video><div className="v3-film-caption"><span>Три Короны · Иссык-Куль</span><strong>Большая история о территории</strong></div></div><div className="wrap v3-territory-route">{territoryJourney.map(([index, title, text]) => <article key={index}><span>{index}</span><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></section>

      <section className="v3-amenities" id="experience" aria-labelledby="amenities-title"><div className="wrap v3-amenities-layout"><div className="v3-amenities-copy"><p className="eyebrow">Что есть на территории</p><h2 className="display-title" id="amenities-title">Не список услуг.<br />Сценарии вашего дня.</h2><p className="lead">Открытый бассейн 15×8 м, SPA, массаж, собственный пляж и 150-метровый пирс складываются в полноценный курортный день без постоянных переездов.</p><a className="button button-dark" href="#booking">Проверить даты</a></div><figure className="v3-amenities-photo"><Image src="/media/three-crowns/lake-night.webp" alt="Иссык-Куль и территория Три Короны" fill sizes="(max-width: 900px) 100vw, 54vw" /></figure></div><div className="wrap v3-amenity-grid">{amenityCards.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></section>

      <section className="v3-reviews" id="reviews" aria-labelledby="reviews-title"><div className="wrap v3-section-head"><div><p className="eyebrow">Отзывы гостей</p><h2 className="display-title" id="reviews-title">Что запоминается<br />после поездки</h2></div><p>Пока не подставляем выдуманные цитаты. Показываем реальные темы, которые чаще всего встречаются в публичных отзывах: расположение у озера, пляж, пирс и удобство курортного формата. Позже подключим актуальные отзывы с именами и источниками.</p></div><div className="wrap v3-review-grid">{reviewThemes.map((review) => <article key={review.score}><span>{review.score}</span><h3>{review.title}</h3><p>{review.text}</p></article>)}</div><div className="wrap v3-extra-services"><div className="v3-extra-heading"><p className="eyebrow light">Дополнительно</p><h3>Отдых не заканчивается территорией</h3><p>Если хотите больше впечатлений или максимально простой маршрут, дополнительные услуги можно согласовать вместе с проживанием.</p></div><div className="v3-extra-grid">{extraServices.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h4>{title}</h4><p>{text}</p></article>)}</div></div></section>

      <section className="v3-groups" id="groups" aria-labelledby="groups-title"><div className="v3-groups-media" aria-hidden="true"><Image src="/media/three-crowns/hero-resort.webp" alt="" fill sizes="100vw" /></div><div className="v3-groups-shade" aria-hidden="true" /><div className="wrap v3-groups-content"><div className="v3-groups-intro"><p className="eyebrow light">Групповые заезды</p><h2 className="display-title light" id="groups-title">Команды, сборы<br />и корпоративный отдых</h2><p>Для организованных групп важен не только номер. Нужно заранее собрать размещение, график, питание, транспорт и коммуникацию с одним ответственным менеджером. Именно так мы предлагаем строить групповой заезд.</p><a className="button button-accent" href="https://wa.me/996558085008" target="_blank" rel="noreferrer">Обсудить групповой заезд</a></div><div className="v3-group-grid">{groupFormats.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div></section>

      <section className="v3-contacts" id="contacts" aria-labelledby="contacts-title"><div className="wrap v3-contact-head"><div><p className="eyebrow">Контакты и дорога</p><h2 className="display-title" id="contacts-title">Чолпон-Ата.<br />Берег Иссык-Куля.</h2><p>Три Короны Resort & SPA находится в Чолпон-Ате по адресу Иманбай Молдо, 722315. Откройте карту, постройте маршрут или свяжитесь с менеджером — поможем с приездом и проживанием.</p></div><div className="v3-contact-actions"><a href="tel:+996558085002"><span>Бронирование</span><strong>+996 558 08 50 02</strong></a><a href="https://wa.me/996558085008" target="_blank" rel="noreferrer"><span>WhatsApp / менеджер</span><strong>+996 558 08 50 08</strong></a><a href="mailto:3koronykg@mail.ru"><span>Email</span><strong>3koronykg@mail.ru</strong></a></div></div><div className="wrap v3-map-layout"><div className="v3-map-card"><iframe title="Три Короны Resort & SPA на Google Maps" src="https://www.google.com/maps?q=Imanbay%20Moldo%2C%20Cholpon-Ata%20722315%2C%20Kyrgyzstan&output=embed" loading="lazy" referrerPolicy="no-referrer-when-downgrade" allowFullScreen /></div><div className="v3-arrival-card"><p className="eyebrow">Перед поездкой</p><h3>Сохраните контакты — остальное поможем организовать</h3><p>Напишите менеджеру, если нужен подбор номера, групповое размещение, трансфер или дополнительная программа по Иссык-Кулю.</p><div><a className="button button-dark" href="#booking">Выбрать даты</a><a className="text-link" href="https://www.google.com/maps/search/?api=1&query=Imanbay%20Moldo%2C%20Cholpon-Ata%20722315%2C%20Kyrgyzstan" target="_blank" rel="noreferrer">Открыть Google Maps ↗</a></div></div></div></section>
