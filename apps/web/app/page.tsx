  ...
  return <>
    ...
    <main id="top">
      ...
      <section className="section rooms-section" id="rooms" aria-labelledby="rooms-title">
        ...
        <div className="wrap accommodation-editorial"><figure className="accommodation-photo"><img src="https://www.3korony.com/assets/images/rooms/93.jpg" alt="Интерьер номера в Трёх Коронах" loading="lazy" /><figcaption><span>Номера «Три Короны»</span><small>Категория и точная конфигурация конкретного номера подтверждаются при бронировании.</small></figcaption></figure><div className="room-list" role="list">{roomCategories.map(([num,title,capacity,note]) => <article className="room-row" key={num} role="listitem"><span className="room-index">{num}</span><div><h3>{title}</h3><p>{capacity} · {note}</p></div><a href="#booking" aria-label={`Проверить наличие: ${title}`}>Наличие <span aria-hidden="true">↗</span></a></article>)}</div></div>
      </section>
      ...
      <section className="section gallery-section" id="gallery" aria-labelledby="gallery-title"><div className="wrap section-heading"><div><p className="eyebrow">Атмосфера</p><h2 className="display-title" id="gallery-title">Озеро, номера,<br />курортный день</h2></div><p className="section-aside">Галерея переходит на собственный медиапакет отеля: корпуса, территория, номера, конференц-зал, бильярд и ночной Иссык-Куль.</p></div><ResortGallery images={galleryImages} /></section>
