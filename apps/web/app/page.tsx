  ...
  return <>
    ...
    <main className="landing-v3" id="top">
      ...
      <section className="v3-rooms" id="rooms" aria-labelledby="rooms-title">
        ...
        <div className="wrap v3-room-grid">{roomCategories.map((room) => <Link className="v3-room-card" href={`/rooms/${room.slug}`} key={room.slug}><div className="v3-room-card-photo"><Image src={getRoomHero(room.slug)} alt={room.name} fill sizes="(max-width: 760px) 88vw, (max-width: 1100px) 44vw, 31vw" /></div><div className="v3-room-card-body"><div className="v3-room-card-top"><span>{room.index}</span><span>{room.capacity} · {room.area}</span></div><h3>{room.name}</h3><p>{room.summary}</p><div className="v3-room-card-price"><small>Высокий сезон</small><strong>{formatKgs(room.rates.peak)} сом / сутки</strong></div><b>Смотреть категорию →</b></div></Link>)}</div>
      </section>
      ...
      <section className="v3-groups" id="groups" aria-labelledby="groups-title">
        ...
        <div className="wrap v3-groups-content"><div className="v3-groups-intro"><p className="eyebrow light">Групповые заезды</p><h2 className="display-title light" id="groups-title">Команды, сборы<br />и корпоративный отдых</h2><p>Для организованных групп важен не только номер. Нужно заранее собрать размещение, график, питание, транспорт и коммуникацию с одним ответственным менеджером. Именно так мы предлагаем строить групповой заезд.</p><a className="button button-accent" href="https://wa.me/996558085002" target="_blank" rel="noreferrer">Обсудить групповой заезд</a></div><div className="v3-group-grid">{groupFormats.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h3>{title}</h3><p>{text}</p></article>)}</div></div>
      </section>

      <section className="v3-contacts" id="contacts" aria-labelledby="contacts-title">
        <div className="wrap v3-contact-head"><div><p className="eyebrow">Контакты и дорога</p><h2 className="display-title" id="contacts-title">Чолпон-Ата.<br />Берег Иссык-Куля.</h2><p>Три Короны Resort & SPA находится в Чолпон-Ате по адресу Иманбай Молдо, 722315. Откройте карту, постройте маршрут или свяжитесь с менеджером — поможем с приездом и проживанием.</p></div><div className="v3-contact-actions"><a href="tel:+996558085002"><span>Бронирование</span><strong>+996 558 08 50 02</strong></a><a href="https://wa.me/996558085002" target="_blank" rel="noreferrer"><span>WhatsApp / менеджер</span><strong>+996 558 08 50 02</strong></a><a href="mailto:3koronykg@mail.ru"><span>Email</span><strong>3koronykg@mail.ru</strong></a></div></div>
