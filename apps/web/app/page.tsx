...
export default function HomePage() {
  ...
  return <>
    ...
    <main className="landing-v3" id="top">
      ...
      <section className="v3-rooms" id="rooms" aria-labelledby="rooms-title"><div className="wrap v3-section-head"><div><p className="eyebrow">Номерной фонд</p><h2 className="display-title" id="rooms-title">Все 12 категорий.<br />Выбирайте спокойно.</h2></div><div><p>От компактных одноместных вариантов до просторных двухкомнатных категорий и апартаментов. Сравните вместимость, площадь и сезонную стоимость, а затем откройте нужную категорию и проверьте даты.</p><Link className="text-link" href="/rooms">Открыть полный каталог →</Link></div></div><div className="wrap v3-room-grid">{roomCategories.map((room) => <Link className="v3-room-card" href={`/rooms/${room.slug}`} key={room.slug}><div className="v3-room-card-photo"><Image src="/media/three-crowns/room-double.webp" alt={room.name} fill sizes="(max-width: 760px) 88vw, (max-width: 1100px) 44vw, 31vw" /></div><div className="v3-room-card-body"><div className="v3-room-card-top"><span>{room.index}</span><span>{room.capacity} · {room.area}</span></div><h3>{room.name}</h3><p>{room.summary}</p><div className="v3-room-card-price"><small>Высокий сезон</small><strong>{formatKgs(room.rates.peak)} сом / сутки</strong></div><b>Смотреть категорию →</b></div></Link>)}</div></section>

      <section className="v3-territory" id="resort" aria-labelledby="territory-title"><div className="wrap v3-section-head light-head"><div><p className="eyebrow light">Территория курорта</p><h2 className="display-title light" id="territory-title">От первого шага<br />до конца пирса</h2></div><p>Пройдите весь маршрут курорта — от размещения и дневного отдыха до собственного пляжа и длинного пирса, который выходит в открытое пространство Иссык-Куля.</p></div><div className="wrap v3-territory-film"><video controls playsInline preload="metadata" poster="/media/three-crowns/hero-resort.webp"><source src="/media/three-crowns/resort-drone.mp4" type="video/mp4" /></video><div className="v3-film-caption"><span>Три Короны · Иссык-Куль</span><strong>Большая история о территории</strong></div></div><div className="wrap v3-territory-route">{territoryJourney.map(([index, title, text]) => <article key={index}><span>{index}</span><div><h3>{title}</h3><p>{text}</p></div></article>)}</div></section>
      ...
      <section className="v3-reviews" id="reviews" aria-labelledby="reviews-title"><div className="wrap v3-section-head"><div><p className="eyebrow">Отзывы гостей</p><h2 className="display-title" id="reviews-title">Что запоминается<br />после поездки</h2></div><p>В отзывах гости чаще всего отмечают расположение у озера, собственный пляж, пирс и возможность провести большую часть дня внутри одного курортного пространства.</p></div><div className="wrap v3-review-grid">{reviewThemes.map((review) => <article key={review.score}><span>{review.score}</span><h3>{review.title}</h3><p>{review.text}</p></article>)}</div><div className="wrap v3-extra-services"><div className="v3-extra-heading"><p className="eyebrow light">Дополнительно</p><h3>Отдых не заканчивается территорией</h3><p>Если хотите больше впечатлений или максимально простой маршрут, дополнительные услуги можно согласовать вместе с проживанием.</p></div><div className="v3-extra-grid">{extraServices.map(([title, text], index) => <article key={title}><span>{String(index + 1).padStart(2, "0")}</span><h4>{title}</h4><p>{text}</p></article>)}</div></div></section>
