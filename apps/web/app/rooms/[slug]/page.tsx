...
export async function generateMetadata({ params, searchParams }: RoomPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [{ url: "/media/three-crowns/room-double.webp", alt: String(copy.imageAlt) }],
    },
  };
}
    ...
    <main className="rooms-page" id="top">
      <section className="room-detail-hero" aria-labelledby="room-detail-title">
        ...
        <div className="wrap room-detail-hero-content">
          ...
          <div className="room-detail-kicker"><span>{localized.capacity}</span><span>{room.area}</span><span>{String(copy.availability)}</span></div>
        </div>
      </section>
