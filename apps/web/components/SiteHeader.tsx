...
const NAV: Record<Locale, { rooms: string; territory: string; amenities: string; reviews: string; groups: string; contacts: string; book: string; menuOpen: string; menuClose: string; home: string; subtitle: string }> = {
  ru: { rooms: "Номера", territory: "Территория", amenities: "На территории", reviews: "Отзывы", groups: "Группам", contacts: "Контакты", book: "Проверить даты", menuOpen: "Открыть меню", menuClose: "Закрыть меню", home: "Три Короны — на главную", subtitle: "Resort & SPA · Issyk-Kul" },
  kg: { rooms: "Номерлер", territory: "Аймак", amenities: "Инфраструктура", reviews: "Пикирлер", groups: "Топторго", contacts: "Байланыш", book: "Даталарды текшерүү", menuOpen: "Менюну ачуу", menuClose: "Менюну жабуу", home: "Үч Таажы — башкы бет", subtitle: "Resort & SPA · Ысык-Көл" },
  en: { rooms: "Rooms", territory: "Resort", amenities: "Facilities", reviews: "Reviews", groups: "Groups", contacts: "Contacts", book: "Check dates", menuOpen: "Open menu", menuClose: "Close menu", home: "Three Crowns — home", subtitle: "Resort & SPA · Issyk-Kul" },
};
...
export default function SiteHeader() {
  ...
  return (
    <header className={`site-header ${scrolled ? "is-scrolled" : ""}`}>
      ...
      <div className={`mobile-menu ${open ? "is-open" : ""}`} id="mobile-menu" aria-hidden={!open}><nav className="wrap" aria-label="Мобильная навигация">{languageControl("mobile-language")}{links.map(([href,label]) => <a key={href} href={href} onClick={closeMenu}>{label}</a>)}<a className="button button-accent" href={withLanguage("/#booking")} onClick={closeMenu}>{copy.book}</a></nav></div>
    </header>
  );
}
