...
export function pmsOwnerRoomDisplayLabel(room: PmsRoomDisplayInput) {
  ...
  if (room.room_type_name === "Одноместный, цоколь") return room.code;

  if (room.room_type_name === "Двухместный стандарт, цоколь") {
    const sleeping = raw.replace("; доп. кровать", "").trim();
    return `${room.code}-цоколь${sleeping ? ` (${sleeping})` : ""}${raw.includes("доп. кровать") ? " — доп. кровать" : ""}`;
  }
  ...
  if (room.room_type_name === "Двухместный стандарт в коттеджном доме") return room.code;

  if (room.code === "311" || room.code === "314") {
    const [sleeping] = raw.split(";");
    return `${room.code}${sleeping?.trim() ? ` (${sleeping.trim()})` : ""} + кухня`;
  }

  if (room.code === "421") return `421 sea view${raw ? ` (${raw})` : ""}`;
