...
type AgentReport = {
  agent: Agent;
  summary: { reservations: number; effective_reservations: number; revenue_kgs: number; room_nights: number; average_check_kgs: number; cancelled_or_no_show: number };
  reservations: Array<{ id: string; booking_number: string; status: string; check_in: string; check_out: string; total_kgs: number; guest_name?: string | null; guest_phone?: string | null; rooms?: string | null; discount_percent: number; extra_bed_count: number }>;
...
export default function AgentsBoard() {
  ...
  useEffect(() => { if (selectedId) void loadReport(selectedId); }, [fromDate, toDate]); // eslint-disable-line react-hooks/exhaustive-deps

  const totals = useMemo(() => items.reduce((acc, item) => ({ reservations: acc.reservations + item.reservations, revenue: acc.revenue + item.revenue_kgs, nights: acc.nights + item.room_nights }), { reservations: 0, revenue: 0, nights: 0 }), [items]);
  ...
  async function createAgent(event: FormEvent) {
    ...
    try {
      const response = await fetch("/core/api/v1/admin/owner-corrections/agents", {
        ...
        body: JSON.stringify({ name: name.trim(), contact_name: contact.trim() || null, phone: phone.trim() || null, whatsapp: whatsapp.trim() || null, email: email.trim() || null, notes: notes.trim() || null }),
      });
      const body = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(typeof body.detail === "string" ? body.detail : "Не удалось создать агента");
      setName(""); setContact(""); setPhone(""); setWhatsapp(""); setEmail(""); setNotes(""); setShowCreate(false);
      await load();
  ...
  async function addInteraction(event: FormEvent) {
    ...
    try {
      ...
      setInteractionNote(""); setNextContactAt("");
  ...
  return <main className="shell">
    <div className="topbar">
      <div><p className="eyebrow">CRM · B2B продажи</p><h1>Агенты</h1><p className="subtitle">Карточки партнёров, брони, выручка, ночи и история контактов. Период отчёта применяется ко всем показателям.</p></div>
    ...
    <section className="summary">
      ...
      <div className="summary-card"><strong>{money(totals.revenue)}</strong><span>Выручка</span></div>
    </section>
    ...
    {loading ? <div className="loading">Загрузка агентов…</div> : <section className="pms-v2-card">
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th>Агент</th><th>Контакт</th><th>Брони</th><th>Ночи</th><th>Выручка</th><th>Последний заезд</th><th /></tr></thead><tbody>
        {items.map((agent) => <tr key={agent.id} style={{ opacity: agent.status === "INACTIVE" ? .55 : 1 }}><td><strong>{agent.name}</strong><br/><small>{agent.status === "ACTIVE" ? "Активен" : "Неактивен"}</small></td><td>{agent.contact_name || "—"}<br/><small>{agent.phone || agent.whatsapp || ""}</small></td><td>{agent.reservations}</td><td>{agent.room_nights}</td><td>{money(agent.revenue_kgs)}</td><td>{agent.last_check_in || "—"}</td><td><button className="btn" onClick={() => void loadReport(agent.id)}>Карточка / отчёт</button></td></tr>)}
      </tbody></table></div>
    </section>}

    {report && <section className="pms-v2-card" style={{ marginTop: 20 }}>
      <div className="topbar"><div><p className="eyebrow">Карточка агента</p><h2>{report.agent.name}</h2><p>{[report.agent.contact_name, report.agent.phone, report.agent.whatsapp, report.agent.email].filter(Boolean).join(" · ")}</p></div><button className="btn" onClick={() => { setReport(null); setSelectedId(null); }}>Закрыть</button></div>
      <section className="summary">
        <div className="summary-card"><strong>{report.summary.effective_reservations}</strong><span>Реальных броней</span></div>
        <div className="summary-card"><strong>{report.summary.room_nights}</strong><span>Ночей</span></div>
        <div className="summary-card"><strong>{money(report.summary.revenue_kgs)}</strong><span>Выручка</span></div>
        <div className="summary-card"><strong>{money(report.summary.average_check_kgs)}</strong><span>Средний чек</span></div>
        <div className="summary-card"><strong>{report.summary.cancelled_or_no_show}</strong><span>Отмен / no-show</span></div>
      </section>

      <h3>Бронирования</h3>
      <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse" }}><thead><tr><th>Бронь</th><th>Гость</th><th>Номер</th><th>Период</th><th>Сумма</th><th>Скидка</th><th>Допмест</th></tr></thead><tbody>{report.reservations.map((item) => <tr key={item.id}><td><strong>{item.booking_number}</strong><br/><small>{item.status}</small></td><td>{item.guest_name || "—"}<br/><small>{item.guest_phone || ""}</small></td><td>{item.rooms || "—"}</td><td>{item.check_in} → {item.check_out}</td><td>{money(item.total_kgs)}</td><td>{item.discount_percent}%</td><td>{item.extra_bed_count}</td></tr>)}</tbody></table></div>
